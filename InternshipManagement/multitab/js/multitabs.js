...
if (typeof jQuery === "undefined") {
    ...
    defaultLayoutTemplates = {
        ...
        default: '<div class="mt-wrapper {mainClass}" style="height: 100%;" >' +
            ...
            '<a href="#"  class="dropdown-toggle" data-bs-toggle="dropdown">{dropdown}<span class="caret"></span></a>' +
        ...
        classic: '<div class="mt-wrapper {mainClass}" style="height: 100%;" >' +
            ...
            '<div class="mt-nav mt-nav-tools-right">' +
            '<ul  class="nav {nav-tabs}">' +
            '<li class="mt-dropdown dropdown nav-item">' +
            '<a href="#"  class="dropdown-toggle nav-link" data-bs-toggle="dropdown">{dropdown}<span class="caret"></span></a>' +
            '<ul role="menu" class="mt-hidden-list dropdown-menu dropdown-menu-end"></ul>' +
            '</li>' +
            '</ul>' +
            '</div>' +
            '</div>' +
        ...
        simple: '<div class="mt-wrapper {mainClass}" style="height: 100%;" >' +
            ...
            '<div class="tab-content mt-tab-content " > </div>' +
            '</div>',
        navTab: '<a data-id="{navTabId}" class="mt-nav-tab" data-type="{type}" data-index="{index}" data-url="{url}">{title} <i style="margin-left:5px" class="mt-close-tab fa fa-times"></i></a>',
        closeBtn: ' ',
    ...
    MultiTabs.prototype = {
        ...
        _createNavTab: function (param) {
            ...
            $el.navPanelList.find('a[data-type="' + param.type + '"][data-index="' + param.index + '"]:first').addClass('nav-link');
        ...
        _listen: function () {
            ...
            handler($el.nav, 'click', '.mt-close-tab', function () {
                ...
                $('.sidebar1 a').removeClass('active'); // Remove active class from all elements in .sidebar1
                var activeNavItem = $('nav.mt-nav ul li.active a[data-url]'); // Get the <a> element inside the active <li> with data-url
                if (activeNavItem.length > 0) { // Check if there is an active element
                    var href = activeNavItem.attr('data-url'); // Get the value of data-url
                    $('.sidebar1 a[href="' + href + '"]').addClass('active'); // Add active class to corresponding .sidebar1 element
                }
                return false; // Avoid potential bugs
            });
                ...
                handler($el.nav, 'click', '.mt-dropdown:not(.open)', function () { //just trigger when dropdown not open.
                    ...
                    if (list) { //when list is not empty
                        while (list.prevList.length) {
                            $dropDown.append(list.prevList.shift().clone().addClass('dropdown-item'));
                        }
                        while (list.nextList.length) {
                            $dropDown.append(list.nextList.shift().clone().addClass('dropdown-item'));
                        }
                    } else {
                        $dropDown.append('<li class="dropdown-item">empty</li>');
                    }
                    // return false; //Avoid possible BUG
                });
            }
